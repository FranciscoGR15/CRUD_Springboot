...
public class UsuarioDAOImplementation implements IUsuario {
    ...
    public Result GetAllFiltrado(String nombre, String apellidoPaterno, String apellidoMaterno, Integer idRol) {
        ...
        jdbcTemplate.execute("{CALL UsuarioDireccionGetAllDinamico(?, ?, ?, ?, ?)}",
                (CallableStatementCallback<Boolean>) callableStatement -> {

                    // Registrar OUT parameter (REF CURSOR)
                    callableStatement.registerOutParameter(5, java.sql.Types.REF_CURSOR);

                    // Setear los parámetros IN del SP
                    callableStatement.setString(1, nombre);
                    callableStatement.setString(2, apellidoPaterno);
                    callableStatement.setString(3, apellidoMaterno);
                    callableStatement.setObject(4, idRol); // puede ser NULL

                    // Ejecutar SP
                    callableStatement.execute();

                    // Obtener ResultSet desde el REF CURSOR
                    ...
                    while (resultSet.next()) {
                        ...
                        if (!result.objects.isEmpty() && idUsuario == ((Usuario) (result.objects.get(result.objects.size() - 1))).getIdUsuario()) {
                            ...
                            Direccion direccion = mapDireccion(resultSet);
                            usuarioActual.direcciones.add(direccion);
    ...
    // Método helper para mapear Direccion
    private Direccion mapDireccion(ResultSet resultSet) throws SQLException {
        Direccion direccion = new Direccion();
        direccion.setIdDireccion(resultSet.getInt("IdDireccion"));
        direccion.setCalle(resultSet.getString("Calle"));
        direccion.setNumeroInterior(resultSet.getString("NumeroInterior"));
    ...
    // Método helper para mapear Usuario
    ...
    @Override
    public Result UsuarioDireccionAddAll(List<Usuario> usuarios) {
        Result result = new Result();

        try {

            jdbcTemplate.batchUpdate("{CALL UsuarioDireccionAddSP(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)}",
                    usuarios, usuarios.size(), (callableStatement, usuario) -> {
    ...
    @Override
    public Result UsuarioStatusUpdateSP(Usuario usuario) {
        Result result = new Result();
